...
export PROJECT_VERSION=3.29.0
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=

set -- "$@" checker-qual:build checker-qual:publish -x framework:compileJava -x checker:compileJava -x checker-qual:javadoc -x checker-qual:signCheckerQualPublication 
...
if [ -z "" ]; then
  echo "Enforce version not set, recreating original tag checker-framework-3.29.0"
  git tag -m checker-framework-3.29.0 -a checker-framework-3.29.0 || true
else
